...
class MazeRoomsDisposableKeysAnnotatedTask(AnnotatedTask):
    def __init__(self, env):
        super(AnnotatedTask, self).__init__()
        self.env = env

        # strips domain and problem file
        self.domain_file = os.path.join(os.path.dirname(__file__), "maze_rooms_disposable_keys.pddl")
    ...
    def _key_unused(self, fact_str):
        key_str = fact_str[1].split("-")
        key_color, key_ind = key_str[1], int(key_str[2])
        key_obj = self.env.keys[key_ind]
        return not key_obj.is_used
...
{GOAL_FLUENTS}
    ...
    with open(problem_file, 'w') as fp:
        ...
        key_at = set()
        key_unused = set()  # all keys are unused
        for key_id in key_ids:
            ...
            if not key_obj.is_used:     # used key won't be listed in init file
                key_unused.add("(key-unused {})".format(key_id))
        KEYAT_FLUENTS = "\n".join(sorted(["\t\t\t" + el for el in key_at]))
        KEYUNUSED_FLUENTS = "\n".join(sorted(["\t\t\t" + el for el in key_unused]))
        ...
        problem_txt = PDDL_PROBLEM_INSTANCE.format(
            ...
            KEYUNUSED_FLUENTS=KEYUNUSED_FLUENTS,
...
if __name__ == "__main__":
    ...
    env = gym.make("MazeRooms-3by3-ThreeDisposableKeys-v0")
    env = env.env  # unwrap
    task = MazeRoomsDisposableKeysAnnotatedTask(env)
