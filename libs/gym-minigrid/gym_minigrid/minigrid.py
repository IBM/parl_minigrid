...
class WorldObj:
    ...
    def decode(type_idx, color_idx, state):
        ...
        if obj_type == 'wall':
            ...
            is_permanant = state == 0
            is_used = state == 2
            v = Key(color, not is_permanant, is_used)
        elif obj_type == 'box':
            v = Box(color)
        elif obj_type == 'door':
    ...
    def toggle(self, env, pos):
        ...
        if self.is_locked:
            if isinstance(env.carrying, Key) and env.carrying.color == self.color:
                if not env.carrying.is_disposable:
                    self.is_locked = False
                    self.is_open = True
                    return True
                else:
                    if not env.carrying.is_used:
                        self.is_locked = False
                        self.is_open = True
                        env.carrying.is_used = True
                        return True
                    else:
                        return False
            return False
...
class Key(WorldObj):
    def __init__(self, color='blue', is_disposable=False, is_used=False):
